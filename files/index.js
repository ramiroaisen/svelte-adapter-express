import { h as httpErrors, o as onFinished$3, d as depd_1, e as encodeurl, s as statuses$2, p as parseurl, a as escapeHtml_1, b as setprototypeof, c as send$2, f as etag_1, r as rangeParser_1, g as fresh_1, i as serveStatic, j as handler } from './handler-6469bbe3.js';
...
import { c as commonjsGlobal, a as commonjsRequire } from './shims-53e326b8.js';
...
bytes$b.exports = bytes$a;
bytes$b.exports.format = format$5;
bytes$b.exports.parse = parse$f;
...
var formatThousandsRegExp$2 = /\B(?=(\d{3})+(?!\d))/g;

var formatDecimalsRegExp$2 = /(?:\.0*|(\.[^0]+)0+)$/;
...
var parseRegExp$2 = /^((-|\+)?(\d+(?:\.\d+)?)) *(kb|mb|gb|tb)$/i;
...
function bytes$a(value, options) {
...
function format$5(value, options) {
  ...
  if (!fixedDecimals) {
    str = str.replace(formatDecimalsRegExp$2, '$1');
  }

  if (thousandsSeparator) {
    str = str.replace(formatThousandsRegExp$2, thousandsSeparator);
  }
...
function parse$f(val) {
  ...
  var results = parseRegExp$2.exec(val);
  var floatValue;
  var unit = 'b';
...
var bytes$9 = bytes$b.exports;
var compressible = compressible_1;
var debug$c = src$3.exports('compression');
...
compression$2.exports.filter = shouldCompress$1;
...
function compression (options) {
  ...
  // options
  var filter = opts.filter || shouldCompress$1;
  var threshold = bytes$9.parse(opts.threshold);
  ...
  return function compression (req, res, next) {
    ...
    onHeaders(res, function onResponseHeaders () {
      ...
      // compression stream
      debug$c('%s compression', method);
      stream = method === 'gzip'
        ? zlib$1.createGzip(opts)
        : zlib$1.createDeflate(opts);
...
function shouldCompress$1 (req, res) {
  var type = res.getHeader('Content-Type');

  if (type === undefined || !compressible(type)) {
    debug$c('%s not compressible', type);
    return false
  }
...
function format$4(value, options) {
  ...
  if (thousandsSeparator) {
    str = str.split('.').map(function (s, i) {
      return i === 0
        ? s.replace(formatThousandsRegExp$1, thousandsSeparator)
        : s
    }).join('.');
  }
...
function getDecoder (encoding) {
  ...
  try {
    ...
    throw createError$3(415, 'specified encoding unsupported', {
      encoding: encoding,
...
function readStream (stream, encoding, length, limit, callback) {
  ...
  if (limit !== null && length !== null && length > limit) {
    return done(createError$3(413, 'request entity too large', {
      expected: length,
      length: length,
      limit: limit,
  ...
  if (stream._decoder || (state && (state.encoding || state.decoder))) {
    // developer error
    return done(createError$3(500, 'stream encoding should not be set', {
  ...
  function onAborted () {
    ...
    done(createError$3(400, 'request aborted', {
      code: 'ECONNABORTED',
      expected: length,
      length: length,
      received: received,
  ...
  function onData (chunk) {
    ...
    if (limit !== null && received > limit) {
      done(createError$3(413, 'request entity too large', {
        limit: limit,
        received: received,
        type: 'entity.too.large'
      }));
  ...
  function onEnd (err) {
    ...
    if (length !== null && received !== length) {
      done(createError$3(400, 'request size did not match content length', {
        expected: length,
        length: length,
        received: received,
...
var onFinished$2 = onFinished$3.exports;
var zlib = require$$7$1;
...
function read$4 (req, res, next, parse, debug, options) {
  ...
  if (opts.encoding === null && encoding !== null && !iconv.encodingExists(encoding)) {
    return next(createError$2(415, 'unsupported charset "' + encoding.toUpperCase() + '"', {
  ...
  getBody(stream, opts, function (error, body) {
    if (error) {
      ...
      if (error.type === 'encoding.unsupported') {
        // echo back charset
        _error = createError$2(415, 'unsupported charset "' + encoding.toUpperCase() + '"', {
          charset: encoding.toLowerCase(),
          type: 'charset.unsupported'
        });
      } else {
        // set status code on error
        _error = createError$2(400, error);
      }

      // read off entire request
      stream.resume();
      onFinished$2(req, function onfinished () {
        next(createError$2(400, _error));
      });
      return
    }
...
function contentstream (req, debug, inflate) {
  ...
  if (inflate === false && encoding !== 'identity') {
    throw createError$2(415, 'content encoding unsupported', {
      encoding: encoding,
      type: 'encoding.unsupported'
    })
  }
  ...
  switch (encoding) {
    ...
    case 'identity':
      ...
      break
    default:
      throw createError$2(415, 'unsupported content encoding "' + encoding + '"', {
        encoding: encoding,
        type: 'encoding.unsupported'
      })
  }
...
var debug$a = src$2.exports('body-parser:json');
var read$3 = read_1;
...
function json (options) {
  ...
  return function jsonParser (req, res, next) {
    ...
    // read
    read$3(req, res, next, parse, debug$a, {
      encoding: charset,
      inflate: inflate,
      limit: limit,
      verify: verify
...
var debug$9 = src$2.exports('body-parser:raw');
var read$2 = read_1;
...
function raw (options) {
  ...
  return function rawParser (req, res, next) {
    ...
    // read
    read$2(req, res, next, parse, debug$9, {
      encoding: null,
      inflate: inflate,
      limit: limit,
      verify: verify
...
var debug$8 = src$2.exports('body-parser:text');
var read$1 = read_1;
...
function text (options) {
  ...
  return function textParser (req, res, next) {
    ...
    // read
    read$1(req, res, next, parse, debug$8, {
      encoding: charset,
      inflate: inflate,
      limit: limit,
      verify: verify
...
var debug$7 = src$2.exports('body-parser:urlencoded');
...
function urlencoded (options) {
  ...
  return function urlencodedParser (req, res, next) {
    ...
    // read
    read(req, res, next, parse, debug$7, {
      debug: debug$7,
      encoding: charset,
      inflate: inflate,
      limit: limit,
...
function extendedparser (options) {
  ...
  return function queryparse (body) {
    ...
    debug$7('parse extended urlencoding');
    return parse(body, {
...
var onFinished$1 = onFinished$3.exports;
var parseUrl$2 = parseurl.exports;
var statuses$1 = statuses$2;
var unpipe = unpipe_1;
...
var isFinished = onFinished$1.isFinished;
...
function finalhandler (req, res, options) {
  ...
  return function (err) {
    ...
    if (err) {
      ...
      // not found
      status = 404;
      msg = 'Cannot ' + req.method + ' ' + encodeUrl$1(getResourceName(req));
    }
...
var debug$3 = src.exports('express:router:layer');
...
var debug$2 = src.exports('express:router:route');
...
var debug$1 = src.exports('express:router');
var deprecate$2 = depd_1('express');
var flatten = arrayFlatten_1;
var parseUrl$1 = parseurl.exports;
...
proto.param = function param(name, fn) {
  ...
  if (name[0] === ':') {
    deprecate$2('router.param(' + JSON.stringify(name) + ', fn): Use router.param(' + JSON.stringify(name.substr(1)) + ', fn) instead');
    name = name.substr(1);
  }
...
proto.handle = function handle(req, res, out) {
  ...
  debug$1('dispatching %s %s', req.method, req.url);
  ...
  function trim_prefix(layer, layerError, layerPath, path) {
    if (layerPath.length !== 0) {
      ...
      debug$1('trim prefix (%s) from url %s', layerPath, req.url);
      removed = layerPath;
    ...
    debug$1('%s %s : %s', layer.name, layerPath, req.originalUrl);
...
proto.use = function use(fn) {
  ...
  for (var i = 0; i < callbacks.length; i++) {
    ...
    // add the middleware
    debug$1('use %o %s', path, fn.name || '<anonymous>');
...
var debug = src.exports('express:view');
var path$2 = path$3;
var fs = require$$3$1;
...
View.prototype.resolve = function resolve(dir, file) {
  ...
  // <path>/index.<ext>
  path = join(dir, basename$1(file, ext), 'index' + ext);
  stat = tryStat(path);
...
var onFinished = onFinished$3.exports;
var path$1 = path$3;
...
exports.static = serveStatic.exports;
...
function shouldCompress (req, res) {
  ...
  return compression$1.filter(req, res)
}

const server = express();
server.use(compression$1({ filter: shouldCompress }));
server.use(handler);
server.listen(3000, () => {
